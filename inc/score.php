...
function vibe_compute_seo_score($post_id){
  ...
  $kw_raw = get_post_meta($post_id,'_vibe_focus_keywords',true);
  $kw = '';
  if ($kw_raw){
    $parts = array_map('trim', explode(',', $kw_raw));
  ...
  if ($kw){
    ...
    $slug  = mb_strtolower($post->post_name);
    $cont  = mb_strtolower( wp_strip_all_tags($post->post_content) );

    if (mb_strpos($title,$kw) !== false) $score += 25;
    if (mb_strpos($slug,$kw)  !== false) $score += 15;
